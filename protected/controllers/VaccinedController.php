...
class VaccinedController extends Controller {
    ...
    public function actionCoverage() {
        ...
        $sql = "SELECT t1.Tambon,t1.subdistname,t1.target,t2.num,ROUND((t2.num/t1.target*100),2) as percent 
                    FROM (
                    SELECT substr(villagecodefull,1,6) as Tambon
                    ,subdistname
                    ,count(*) target
                    FROM tmp_epi_4audit 
                    -- need params
                    WHERE birth BETWEEN '2013-01-01' AND '2013-03-31'
                    GROUP BY subdistname
                    ) as t1
                    LEFT JOIN 
                    (
                    SELECT substr(villagecodefull,1,6) as Tambon
                    ...
                    FROM tmp_epi_4audit 
                    -- need params
                    WHERE birth BETWEEN '2013-01-01' AND '2013-03-31' AND full1='1'
                    GROUP BY subdistname
                    ) as t2
                    ON t1.Tambon=t2.Tambon 
                    ORDER BY Tambon";
